...
\begtt cmr5 CMR5 <cmr5.pfb \endtt
...
system TDS roots. `TEXMFUSER`'s default value is `$XDG_DATA_HOME/{mmtex,texmf}`
which usually is `~/.local/share/mmtex` and `~/.local/share/texmf`.
`TEXMFSYSTEM` % fix $ syntax highlting defaults to
`/usr/local/share/{mmtex,texmf}` and `/usr/share/{mmtex,texmf}`. The user (or
system administrator) can customize these at any level they want. The order is
important here, because earlier directories are search first and we want to
prioritize user files over system files.

\MMTeX\ core files are always available and preferred over any other. Usually
the user doesn't want to override basic functionality and if they really do,
they can always use `TEXMFDOTDIR`.
